...
describe('PeopleXdClient', () => {
    ...
    it('should be instantiated correctly', async () => {
        ...
        expect(TokenManager.new).toHaveBeenCalledTimes(1) // Ensure TokenManager was called once
        expect(TokenManager.new).toHaveBeenCalledWith(url, clientId, clientSecret)
    })
    ...
    it('should fetch appointments data correctly', async () => {
        ...
        TokenManager.new = jest.fn().mockResolvedValue(mockTokenManagerInstance)
        PeopleXdClient.tokenManager = await TokenManager.new(url, clientId, clientSecret)
