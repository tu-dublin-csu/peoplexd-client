...
describe('PeopleXdClient', () => {
    ...
    beforeEach(() => {
        client = new PeopleXdClient(url, clientId, clientSecret)
    })
    ...
    it('should be instantiated correctly', () => {
        ...
        expect(TokenManager).toHaveBeenCalledTimes(1) // Ensure TokenManager was called once
        expect(TokenManager).toHaveBeenCalledWith(url, clientId, clientSecret)
    })
    ...
    it('should fetch appointments data correctly', async () => {
        ...
        expect(TokenManager.prototype.useOrFetchToken).toHaveBeenCalledTimes(1)
    })
